...
function removeTasksDataColumnPrefixes(entities: ComponentFramework.WebApi.Entity[]) {
    return entities.map((entity: ComponentFramework.WebApi.Entity) => {
        return {
            ...
            parentId               : entity[`${databasePrefix}parentid`],
            orderedParentIndex     : entity[`${databasePrefix}orderedParentIndex`],
...
const BryntumGanttComponent: FunctionComponent<IBryntumGanttComponentProps> = (
    ...
    const disableCreateRef = useRef(false);
    const disableDeleteRef = useRef(false);
    ...
    const fetchRecords = async() => {
        ...
        `?$orderby=${databasePrefix}parentindex asc,${databasePrefix}orderedparentindex asc`
            );
    ...
    const syncData = ({ store, action, records }: SyncData) => {
        ...
        if (storeId === 'tasks') {
            if (action === 'add') {
                if (records.length > 1 || disableCreateRef.current) return;
                try {
                  disableCreateRef.current = true;
                  setTimeout(() => {
                      disableCreateRef.current = false;
                  }, 50);
      
                    const newTask = records[0] as GanttTask;
                    if (JSON.stringify(newTask.meta.modified) === "{}") return;
                    const modifiedKeys = Object.keys(newTask.meta.modified);
                    if (modifiedKeys.includes('orderedParentIndex')) return;
            ...
            newTask.inactive !== undefined && newTask.inactive !== null
                ...
                : newTask.inactive,
                        ...
                        [`${databasePrefix}parentid`] : newTask.parentId,
                        [`${databasePrefix}orderedparentindex`] : newTask.orderedParentIndex,
            ...
            if (action === 'remove') {
                ...
                if (disableDeleteRef.current) return;
            ...
            if (action === 'update') {
                ...
                task.inactive !== undefined && task.inactive !== null
                    ...
                    : task.inactive,
                            ...
                            [`${databasePrefix}parentid`] : task.parentId,
                            [`${databasePrefix}orderedparentindex`] : task.orderedParentIndex,
                            [`${databasePrefix}calendar`] :
    ...
    const cellClick = ({ target }: {target: HTMLElement}) => {
      if (target.className === 'b-tree-expander b-icon b-icon-tree-collapse' || target.className === 'b-tree-expander b-icon b-icon-tree-expand') {
        disableCreateRef.current = true;
        disableDeleteRef.current = true;
        setTimeout(() => {
          disableCreateRef.current = false;
          disableDeleteRef.current = false;
      }, 50);
      }
    }
